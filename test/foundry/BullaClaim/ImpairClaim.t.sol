...
import {Claim, Status, ClaimBinding, CreateClaimParams, ClaimMetadata, LockState, CancelClaimApproval, ImpairClaimApproval} from "contracts/types/Types.sol";
...
contract TestImpairClaim is BullaClaimTestHelper {
    ...
    function testImpairClaim_Success() public {
        ...
        CreateClaimParams memory params = new CreateClaimParamsBuilder()
            ...
            .withDueBy(block.timestamp + 30 days)
            .build();
    ...
    function testImpairClaim_EdgeCases() public {
        ...
        CreateClaimParams memory params = new CreateClaimParamsBuilder()
            .withCreditor(creditor)
            .withDebtor(debtor)
            .withClaimAmount(uint256(type(uint128).max))
            .withToken(address(weth))
            .withDueBy(block.timestamp + 30 days)
            .build();
    ...
    function test_FUZZ_impairClaim(
        uint256 pk,
        address _debtor,
        uint128 claimAmount,
        address token,
