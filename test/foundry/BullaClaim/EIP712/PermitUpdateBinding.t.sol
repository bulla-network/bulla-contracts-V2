...
/// @notice SPEC:
/// This function can approve an controller to update the binding on claims given the following conditions:
...
/// This function can approve an controller to update a claim's binding given:
///     AB1: 0 < `approvalCount` < type(uint64).max -> otherwise reverts
/// This function can revoke an controller's approval to update a claim's binding given:
///     RB1: approvalCount == 0
///
...
contract TestPermitUpdateBinding is Test {
    ...
    function testPermit(uint64 approvalCount) public {
        ...
        bytes memory signature =
            sigHelper.signUpdateBindingPermit({pk: alicePK, user: alice, controller: bob, approvalCount: approvalCount});
        ...
        (,, UpdateBindingApproval memory approval,,,) = approvalRegistry.getApprovals(alice, bob);
    ...
    function testPermitEIP1271(uint64 approvalCount) public {
        ...
        bytes32 digest =
            sigHelper.getPermitUpdateBindingDigest({user: alice, controller: bob, approvalCount: approvalCount});
    ...
    function testRevoke() public {
        ...
        approvalRegistry.permitUpdateBinding({
            ...
            controller: bob,
            approvalCount: 1,
            signature: sigHelper.signUpdateBindingPermit({pk: alicePK, user: alice, controller: bob, approvalCount: 1})
        });

        bytes memory signature =
            sigHelper.signUpdateBindingPermit({pk: alicePK, user: alice, controller: bob, approvalCount: 0});

        vm.expectEmit(true, true, true, true);
        emit UpdateBindingApproved(alice, bob, 0);

        approvalRegistry.permitUpdateBinding({user: alice, controller: bob, approvalCount: 0, signature: signature});
    ...
    function testRevokeEIP1271() public {
        ...
        bytes32 digest = sigHelper.getPermitUpdateBindingDigest({user: alice, controller: bob, approvalCount: 1});
        eip1271Wallet.sign(digest);

        approvalRegistry.permitUpdateBinding({user: alice, controller: bob, approvalCount: 1, signature: bytes("")});

        digest = sigHelper.getPermitUpdateBindingDigest({user: alice, controller: bob, approvalCount: 0});
        eip1271Wallet.sign(digest);

        vm.expectEmit(true, true, true, true);
        emit UpdateBindingApproved(alice, bob, 0);

        approvalRegistry.permitUpdateBinding({user: alice, controller: bob, approvalCount: 0, signature: bytes("")});
    ...
    function testPermitRegisteredContract() public {
        ...
        IBullaControllerRegistry(bullaClaim.approvalRegistry().controllerRegistry()).setControllerName(bob, "bobby bob");

        approvalRegistry.permitUpdateBinding({
            user: alice,
            controller: bob,
            approvalCount: 10,
            signature: sigHelper.signUpdateBindingPermit({pk: alicePK, user: alice, controller: bob, approvalCount: 10})
        });
    }
    ...
    function testCannotReplaySig() public {
        ...
        approvalRegistry.permitUpdateBinding({
            ...
            controller: bob,
            approvalCount: 0,
            signature: sigHelper.signUpdateBindingPermit({pk: alicePK, user: alice, controller: bob, approvalCount: 0})
        });
    ...
    function testCannotPermitThe0Address() public {
        ...
        assertEq(
            ecrecover(sigHelper.getPermitUpdateBindingDigest(user, controller, approvalCount), v, r, s),
            user,
    ...
    function test_fuzz_permitCancelClaim(uint256 pk, uint256 controllerPK, uint64 approvalCount, bool registerContract)
        public
    {
        ...
        if (registerContract) {
            IBullaControllerRegistry(bullaClaim.approvalRegistry().controllerRegistry()).setControllerName(
                controller, "BullaFake"
            );
        }
