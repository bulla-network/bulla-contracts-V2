...
contract TestBullaInvoiceInterest is Test {
    ...
    function testNoInterestBeforeDueDate() public {
        ...
        assertEq(invoice.interestComputationState.latestPeriodNumber, 0, "Period number should still be 0 before due date");
    }
    ...
    function testCompoundInterestMultiplePeriods() public {
        ...
        assertEq(invoice1.interestComputationState.latestPeriodNumber, 10, "Period number should be 10 after 10 periods");
        ...
        assertEq(invoice2.interestComputationState.latestPeriodNumber, 20, "Period number should be 20 after twenty periods");
    ...
    function testInterestPaymentPreservesPeriod() public {
        ...
        assertEq(invoiceAfter.interestComputationState.latestPeriodNumber, periodNumberBefore, "Period number should remain unchanged after payment");
        assertEq(invoiceAfter.interestComputationState.accruedInterest, 0, "Accrued interest should be zero after full interest payment");
    }
    ...
    function testDifferentCompoundingPeriods() public {
        ...
        CreateInvoiceParams memory params1 = new CreateInvoiceParamsBuilder()
            .withDebtor(debtor)
            .withToken(address(token))
            .withDueBy(dueBy)
            .withLateFeeConfig(quarterlyConfig)
            .build();
        ...
        CreateInvoiceParams memory params2 = new CreateInvoiceParamsBuilder()
            .withDebtor(debtor)
            .withToken(address(token))
            .withDueBy(dueBy)
            .withLateFeeConfig(monthlyConfig)
            .build();
        ...
        assertEq(invoice1.interestComputationState.latestPeriodNumber, 4, "Period number should be 4 after 1 year with quarterly compounding");
        
        // Check interest with monthly compounding
        Invoice memory invoice2 = bullaInvoice.getInvoice(invoiceId2);
        assertEq(invoice2.interestComputationState.latestPeriodNumber, 12, "Period number should be 12 after 1 year with monthly compounding");
    ...
    function testZeroInterestConfig() public {
        ...
        CreateInvoiceParams memory params = new CreateInvoiceParamsBuilder()
            ...
            .withToken(address(token))
            .withDueBy(dueBy)
            .withLateFeeConfig(zeroInterestConfig)
            .build();
